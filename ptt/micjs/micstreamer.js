...
// AI: this is the function that actually writes the stream
// AI: e.g. on 'creation' "    this._write(1, meta, this.id);" 
// AI: or on 'new_data'   "    var out = this._write(2, data, this.id);"
// AI: or on 'pause'   	  "  this._write(3, null, this.id);" 
// AI: or on 'resume'	  "  this._write(4, null, this.id);" 
// AI: or on 'end'   	  "  this._write(5, null, this.id);" 
// AI: or on 'destroy'	  "  this._write(6, null, this.id);" 
// AI: THE ACTUALL MESSAGE IS FORMED WITH "  var message = util.pack([code, data, bonus]);"
...
function AudioClient(socket, options) {
  if (!(this instanceof AudioClient)) return new AudioClient(socket, options);
  ...
  if(typeof socket === 'string') {
    this._nextId = 0;
    this._socket = new WebSocket(socket,'audio-streaming-protocol');
  } else {
...
util.inherits(AudioClient, EventEmitter);

AudioClient.prototype.send = function(data, meta){
...
AudioClient.prototype._receiveStream = function(streamId){
  var self = this;
...
AudioClient.prototype.createStream = function(meta){
...
AudioClient.prototype.close = AudioClient.prototype.destroy = function() {
  this._socket.close();
};
...
(function(window) {
  var client = new AudioClient('wss://verbumware.org:8008');
  ...
  var ws=client._socket;
  ws.onmessage = function (message) {
      console.log(message.data);
      //$('fieldset').append($('<p>', { text: message.data }));
//      $('fieldset').append($('<p>', { text: message.data }));
      $("span.treport").text( message.data);
      //window.getElementById('myTextarea').value = { text: message.data };
  };
...
/*
    window.startRecording = function() {
      ...
      //AI: actually here we shall test if the stream exists and resume paused if needed
    }
...
*/
    ...
    function success(e) {
      ...
      //context.sampleRate=44100 on CONCEPTACULUM

      audioInput = context.createMediaStreamSource(e);

      console.log('The sample rate is '+context.sampleRate);

      // acoustic_buffer from 256 to 16384
      var bufferSize = 1024; //256;//4096;
      
      //var meta=Uint16Array(2); meta[0]=context.sampleRate;meta[1]=bufferSize;

      window.Stream = client.createStream(context.sampleRate);
      ...
      recorder.onaudioprocess = function(e){
        ...
        //window.Stream2.write(convertoFloat32ToInt16(left));
	//websocket.send(convertoFloat32ToInt16(left));
      }
